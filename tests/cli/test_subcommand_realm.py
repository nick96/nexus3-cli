...
def test_active(cli_runner, mocker, mock_nexus_client, faker):
    """Test that the command calls the API and handles response correctly"""
    x_active = faker.words(unique=True)
    mock_nexus_client.security_realms.active = x_active
    mocker.patch('nexuscli.cli.util.get_client', return_value=mock_nexus_client)
...
def test_available(cli_runner, mocker, mock_nexus_client, faker):
    """Test that the command calls the API and handles response correctly"""
    x_available = faker.words(unique=True)
    mock_nexus_client.security_realms.list = x_available
    mocker.patch('nexuscli.cli.util.get_client', return_value=mock_nexus_client)
...
def test_activate(cli_runner, mocker, mock_nexus_client, faker):
    """Test that the command calls the API with the given user parameter"""
    x_realm_id = faker.pystr()
    mock_nexus_client.security_realms.activate = mocker.Mock(return_value=x_realm_id)
    mocker.patch('nexuscli.cli.util.get_client', return_value=mock_nexus_client)

    cmd = f'security realm activate {x_realm_id}'
    result = cli_runner.invoke(nexus_cli, cmd, catch_exceptions=False)

    mock_nexus_client.security_realms.activate.assert_called_with(x_realm_id)
    assert result.exit_code == exception.CliReturnCode.SUCCESS.value
